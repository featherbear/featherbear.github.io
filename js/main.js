...
var coverImage = {
  ...
  changeBackground: function(i) {
    i = i === undefined ? this.uniqueImage() : i;
    if (this.usedImages.length == imgMax - 1) this.usedImages = [];
    this.usedImages.push(i);
    var img = new Image();
    img.src = "images/bg/intro-bg (" + i + ").jpg";
    img.onload = function() {
      $("#intro").css("background-image", "url('" + img.src + "')")
    };
  },
}

coverImage.changeBackground(((r = parseInt((q = location.toString()).substr(q.indexOf("?") + 1))) >= imgMin && r <= imgMax) ? r : undefined);
var bgID = setInterval(function() {
  coverImage.changeBackground()
}, 8 * 1000);
